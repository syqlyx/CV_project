        ...
        for i in range(100):
            ...
            if len(self.tracks) > 0:
                ...
                for im, (x,y), flag in zip(self.tracks, np.float32(p1).reshape(-1,2), good):
                    ...
                    if len(im) > self.track_len:#对于存储过长序列的某个点,及时清除离开图像的部分;也可以解决新增跟踪点重复的问题
                        del im[0]
            ...
            if i % self.N == 0:#添加新的追踪点
                ...
                if feature is not None:  
                    if len(self.tracks) > 0:
                        print([vec[-1] for vec in self.tracks])
                        for x, y in np.float32(feature).reshape(-1, 2):
                            if (x, y) in [(int(vec[-1][0]), int(vec[-1][1])) for vec in self.tracks]:
                                    continue
                            else:
...
def main():
    ...
    try:
        ...
        video_path = "/media/vickylzy/文件共享盘/A3_grade_one/class/computer_vision/Assignment/Assignment2/100208AA_out.mp4"
